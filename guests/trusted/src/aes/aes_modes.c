...
#define via_cwd(cwd, ty, dir, len) \
	unsigned long *cwd = (dir
##_##ty##_table + ((len - 128) >> 4))
	...
	AES_RETURN aes_test_alignment_detection(unsigned int n) /* 4 <= n <= 16 */ 
	{
		...
		int nb = len >> 4;
			...
			uint_8t *ksp = (uint_8t *) (ctx->ks);
				...
				aligned_auto(uint_8t, buf,
					    ...
					    (nb >
					     BFR_BLOCKS ? BFR_BLOCKS : nb);
					 ip =
					    (ALIGN_OFFSET(ibuf, 16) ? buf :
					     ibuf);
					op =
					    (ALIGN_OFFSET(obuf, 16) ? buf :
					     obuf);
		...
		int nb = len >> 4;
			...
			uint_8t *ksp = (uint_8t *) (ctx->ks), *ivp = iv;
				...
				via_cbc_op7(ksp, cwd, ibuf, obuf, nb, ivp,
					     ivp);
		...
		int cnt = 0, b_pos = (int)ctx->inf.b[2], nb;
				...
				int m;
				uint_8t * ksp = (uint_8t *) (ctx->ks), *ivp =
				    iv;
				...
				&& !ALIGN_OFFSET(iv, 4))
			     ...
			     && aes_encrypt(iv, iv, ctx) != EXIT_SUCCESS)
