 ...
 */ 
 static buddy_block_t *buddy_add_free(buddy_ctxt_t * buddy, int index,
					int order) 
{
...
  BOOL buddy_contains_order(buddy_ctxt_t * buddy, int order,
			       BOOL or_larger)
{
		     ...
		     uint32_t last_free )
{
	    ...
	    buddy->all =
	    (buddy_block_t *) malloc(size * sizeof(buddy_block_t));
