...
 static Uns32 work_regs1[REGISTER_COUNT_ALL],
...
void doShowWhere(icmProcessorP processor) 
{
		...
		print_stack_frame(processor, tmp,
...
 void do_list_program(icmProcessorP processor, Uns32 pos, int len,
			Uns32 mark) 
{
...
  void doShowRegisters(icmProcessorP processor) 
{
		...
		 for (i = REGISTERS_BANKED_START; i < REGISTERS_BANKED_END;
			i++) {
