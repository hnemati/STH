...
 void test_l2_unmap_pt() 
{
	    ...
	    ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_SECTION, va_section, pga, attrs);
	    ...
	    ISSUE_DMMU_HYPERCALL(CMD_UNMAP_L1_PT_ENTRY, va_section, 0, 0);
...
*/ 
	    ...
	    // creating L2
	    res =
	    ISSUE_DMMU_HYPERCALL(CMD_CREATE_L2_PT, va2pa(va + 0x10000), 0, 0);
	expect(++t_id, "Successful creating a new L2", SUCCESS, res);
	 res =
	    ISSUE_DMMU_HYPERCALL(CMD_CREATE_L2_PT, va2pa(va + 0x20000), 0, 0);
	expect(++t_id, "Successful creating a new L2", SUCCESS, res);
	 
	    // Mapping the first L2 in the current L1 as a second level translation table
	    res =
	    ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_PT, va, va2pa(va + 0x10000),
				 0x21);
	    ...
	    res =
	    ISSUE_DMMU_HYPERCALL(CMD_FREE_L2, va2pa(va + 0x20000), 0, 0);
	    ...
	    // unmap the section
	    res =
	    ISSUE_DMMU_HYPERCALL(CMD_UNMAP_L1_PT_ENTRY, 0xC0400000, 0, 0);
...
*/ 
	    ...
	    // creating L2
	    res =
	    ISSUE_DMMU_HYPERCALL(CMD_CREATE_L2_PT, va2pa(l2_va_1), 0, 0);
	    ...
	    //--
	    res =
	    ISSUE_DMMU_HYPERCALL(CMD_CREATE_L2_PT, va2pa(l2_va_3), 0, 0);
	expect(++t_id, "Successful creating a new L2", SUCCESS, res);
	 
	    // Mapping the L2 PTs in the current L1 as a second level translation table
	    res =
	    ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_PT, va_1, va2pa(l2_va_1), 0x21);
	expect(++t_id,
		"Mapping the first L2 table page in  the the active L1",
		SUCCESS, res);
	 res =
	    ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_PT, va_2, va2pa(l2_va_2), 0x21);
